...
import (
	...
	"github.com/aws/aws-lambda-runtime-interface-emulator/internal/lambda/agents"
	...
	return xray.CaptureInvokeSegment(ctx, xray.WithErrorCause(ctx, appCtx, func(ctx context.Context) error {
		...
		if !execCtx.initDone {
			...
			if err := xray.CaptureInitSubsegment(ctx, func(ctx context.Context) error {
				...
				sendInvokeStartLogEvent(execCtx, invokeRequest.ID, invokeRequest.TenantID, telemetryTracingCtx)
				return err
			}
		...
		runtimeDoneEventData := interop.InvokeRuntimeDoneData{
			...
			TenantID:        invokeRequest.TenantID,
		}
...
func sendInvokeStartLogEvent(execCtx *rapidContext, invokeRequestID string, tenantID interop.TenantID, tracingCtx *interop.TracingCtx) {
